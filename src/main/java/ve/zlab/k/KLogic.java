...
//    }
    ...
    public static SelectDataExtracted extractSelectData(final String select) throws KException {
        ...
        if (c.length() > 1 && c.charAt(0) == '(' && c.charAt(c.length() - 1) == ')') {
            throw new KException(Response.Status.INTERNAL_SERVER_ERROR, "'Select: \"" + c + "\"' format not valid >> Can't start and end with parenthesis");
        }
        
        final String[] tokens = c.split(" ");
        
        if (tokens.length == 0) {
            throw new KException(Response.Status.INTERNAL_SERVER_ERROR, "'Select' format not valid >> Can't be empty");
        }
        ...
        for (int i = tokens.length - 1; i >= 0; i--) {
            ...
            if (tokenUp.equals("AS")) {
                if (i == tokens.length - 1) {
                    throw new KException(Response.Status.INTERNAL_SERVER_ERROR, "'Select: \"" + c + "\"' format not valid >> '" + token + "' Can't be the last word");
                }
                
                if (i == 0) {
                    throw new KException(Response.Status.INTERNAL_SERVER_ERROR, "'Select: \"" + c + "\"' format not valid >> '" + token + "' Can't be the first word");
                }
...
//        }
//        
//        throw new KException(Response.Status.INTERNAL_SERVER_ERROR, StringUtils.join(new String[]{
//            "'", c, "' Format not valid!"
//        }, " "));
//    }
...
//    }
    ...
    public static String buildInsertIntoFrom(final KQuery kQuery) throws KException {
        ...
        if (kQuery.getTable() == null) {
            throw new KException(Response.Status.INTERNAL_SERVER_ERROR, "A 'insert into' table is required");
        }
        
        final List<String> insertInto = kQuery.getInsertInto();
        
        if (insertInto == null || insertInto.isEmpty()) {
            throw new KException(Response.Status.INTERNAL_SERVER_ERROR, "At least one 'insert into' column is required");
        }
    ...
    private static String buildClauseFromInsertInto(final KQuery kQuery) throws KException {
        ...
        if (froms == null) {
            throw new KException(Response.Status.INTERNAL_SERVER_ERROR, "A 'from' table is required");
        }
        
        if (froms.size() > 1) {
            throw new KException(Response.Status.INTERNAL_SERVER_ERROR, "A single 'from' table is required");
        }
